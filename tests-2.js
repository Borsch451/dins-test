...
testCase('/GET posts with only a title', function(){
      it('it should GET all the posts with title=qui est esse', (done) => {
        chai.request('https://jsonplaceholder.typicode.com')
            .get('/posts?title=qui est esse')
            .end((err, res) => {
                res.should.have.status(200);
                res.body.should.be.a('array').that.contains.something.like({title:'qui est esse'});
                done();
            });
      });
  });