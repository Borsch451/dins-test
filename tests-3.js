...
testCase('/GET posts with only a userId', function(){
      it('it should GET all the posts with userId=1', (done) => {
        chai.request('https://jsonplaceholder.typicode.com')
            .get('/posts?userId=1')
            .end((err, res) => {
                res.should.have.status(200);
                res.body.should.be.a('array').that.contains.something.like({userId:1});
                done();
            });
      });
  });